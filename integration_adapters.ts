...
export function CanvasWriterAdapter({ children }: CanvasWriterAdapterProps) {
  const { 
    currentProject, 
    goals, 
    files, 
...
Health Score: ${currentProject.health_score}/100
...
/*
// Canvas Writer Integration Example:
<CanvasWriterAdapter>
  {(canvasProps) => (
    <YourExistingCanvasWriter 
      enhancedPrompt={canvasProps.generateEnhancedPrompt('create a landing page')}
      onCodeGenerated={(code) => canvasProps.saveGeneratedCode('landing.tsx', code)}
      projectContext={canvasProps.getProjectContext()}
    />
  )}
</CanvasWriterAdapter>

// Fractal Auditor Integration Example:  
<FractalAuditorAdapter>
  {(auditorProps) => (
    <YourExistingFractalAuditor
      onRunAudit={() => auditorProps.runProjectAudit('full')}
      projectRules={auditorProps.getProjectRules()}
      globalCoherence={auditorProps.checkGlobalCoherence}
    />
  )}
</FractalAuditorAdapter>

// Superintelligent Platform Integration Example:
<SuperintelligentAdapter>
  {(superProps) => (
    <YourExistingSuperPlatform
      onAnalyzeGoals={() => superProps.analyzeProjectGoals()}
      onCrystallizeGoal={(goal) => superProps.crystallizeGoal(goal)}
      projectPattern={superProps.detectProjectPattern()}
    />
  )}
</SuperintelligentAdapter>
*/